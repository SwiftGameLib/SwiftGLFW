...
public extension GLFW {
    ...
    enum InitHint {
        ...
        internal func glfwHintAndValue() -> (Int32, Int32) {
            ...
            case .JoystickHat(let value):
                return (GLFW_JOYSTICK_HAT_BUTTONS, value.glfwValue)
            case .CocoaCurrentDirectory(let value):
                return (GLFW_COCOA_CHDIR_RESOURCES, value.glfwValue)
            case .CocoaMenubar(let value):
                return (GLFW_COCOA_MENUBAR, value.glfwValue)
            }
        }
    }
    ...
    static func initHint(_ hint: InitHint) {
