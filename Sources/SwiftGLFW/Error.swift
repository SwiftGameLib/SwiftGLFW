...
private var GLFWErrorCallback: GLFW.errorFunc? = nil
...
/// - Note: The corresponding C function is [glfwGetError](https://www.glfw.org/docs/latest/group__init.html#ga944986b4ec0b928d488141f92982aa18)
    @discardableResult static func error() -> (errorCode: Int, errorDescription: String?)? {
    ...
    typealias errorFunc = (_ errorCode: Int, _ errorDescription: String?) -> Void
    ...
    @discardableResult static func setErrorCallback(callback: errorFunc?) -> errorFunc? {
        let previousCallback = GLFWErrorCallback
        GLFWErrorCallback = callback
        
        if GLFWErrorCallback == nil {
            glfwSetErrorCallback(nil)
        } else {
            glfwSetErrorCallback { (errorCode, errorDescription) in
                GLFWErrorCallback!(Int(errorCode), errorDescription != nil ? String(cString: errorDescription!) : nil)
            }
        }
